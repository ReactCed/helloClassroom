...
import {setFile, setShowUpdateNameModal, setShowUpdateEmailModal, setShowUpdateAddressModal, uploadImageSlice, getRecordSlice} from '../../store/slice/studManageAccSlice';
import StudHeader from '../../components/StudHeader';
import StudSidebar from '../../components/StudSidebar';
import '../../assets/css/StudManageAcc.css';
...
function StudManageAcc () {
    const navigate = useNavigate();
    const dispatch = useDispatch();

    const classUsersID = localStorage.getItem('classUsersID');

    const file = useSelector((state: RootState) => state.studManageAcc.file);
    const classUsersImage = useSelector((state: RootState) => state.studManageAcc.classUsersImage);
    const classUsersName = useSelector((state: RootState) => state.studManageAcc.classUsersName);
    const classUsersEmail = useSelector((state: RootState) => state.studManageAcc.classUsersEmail);
    const classUsersAddress = useSelector((state: RootState) => state.studManageAcc.classUsersAddress);
    const classUsersRole = useSelector((state: RootState) => state.studManageAcc.classUsersRole);
    const showUpdateNameModal = useSelector((state: RootState) => state.studManageAcc.showUpdateNameModal);
    const showUpdateEmailModal = useSelector((state: RootState) => state.studManageAcc.showUpdateEmailModal);
    const showUpdateAddressModal = useSelector((state: RootState) => state.studManageAcc.showUpdateAddressModal);
